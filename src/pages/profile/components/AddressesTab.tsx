  ...
  return (
    <div className="rounded-[1.6rem] bg-[linear-gradient(180deg,rgba(27,8,10,0.95),rgba(12,3,4,0.96))] p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="font-cormorant text-4xl text-white">{t("myAddresses")}</p>
          <p className="mt-2 max-w-2xl text-[#d8beb8]">{t("addressesDesc")}</p>
        </div>
        <div className="inline-flex items-center gap-3 rounded-2xl border border-white/8 bg-[#140709] px-4 py-3">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-[#b7918a]">{t("savedAddresses")}</p>
      ...
      <div className="mt-6 grid gap-5">
        <section className="rounded-[1.4rem] border border-white/8 bg-[#120607] p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="font-cormorant text-3xl text-white">{editingAddressId ? t("editAddress") : t("newAddress")}</p>
              <p className="mt-1 text-sm text-[#cfafa8]">{t("addressFormDesc")}</p>
            </div>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-[#f1d5cb]">{t("title")}</span>
              <input
                {...register("title")}
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-[#f1d5cb]">{t("city")}</span>
              <input
                {...register("city")}
            ...
            <label className="block sm:col-span-2">
              <span className="mb-2 block text-sm font-medium text-[#f1d5cb]">{t("addressLine")}</span>
              <input
            ...
            <div className="sm:col-span-2 rounded-2xl border border-[#4b2326] bg-[#1a090c] px-4 py-3">
              <p className="text-xs uppercase tracking-[0.18em] text-[#cda39c]">{t("mapPinLabel")}</p>
            ...
            <label className="block sm:col-span-2">
              <span className="mb-2 block text-sm font-medium text-[#f1d5cb]">{t("notes")}</span>
              <textarea
                {...register("notes")}
        ...
        <section className="overflow-hidden rounded-[1.4rem] border border-[#4b2326] bg-[#120607]">
          <div className="flex items-center justify-between gap-3 border-b border-[#4b2326] px-5 py-4">
            <div>
              <p className="font-cormorant text-3xl text-white">{t("addressPicker")}</p>
              <p className="text-sm text-[#cfafa8]">{t("addressPickerDesc")}</p>
            </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
                <div>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {address.is_primary ? (
                      <span className="rounded-full bg-[#3a1d0f] px-2.5 py-1 text-xs uppercase tracking-[0.16em] text-[#ffd59a]">{t("primaryLabel")}</span>
                    ) : null}
                    {isSameCheckoutAddress(address, preferredCheckoutAddress) ? (
                      <span className="rounded-full bg-[#10241a] px-2.5 py-1 text-xs uppercase tracking-[0.16em] text-[#9ef0c2]">{t("checkoutLabel")}</span>
                    ) : null}
                  </div>
                  <p className="mt-3 text-base text-[#f1d5cf]">{address.address_line}</p>
                  <p className="mt-1 text-sm text-[#c4a39b]">{address.city ?? t("cityNotSet")}</p>
                  {address.notes ? (
                    <div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3">
                      <p className="text-xs uppercase tracking-[0.18em] text-[#cda39c]">{t("notes")}</p>
