 ...
 * PERFORMANCE NOTES
 * ─────────────────────────────────────────────────────────────────────────
 * 1. `hoveredCard` lived in the parent's state, so hovering ANY card
 *    re-rendered the whole page and re-ran `renderBouquetCard` for every
 *    item already loaded (which grows unbounded with infinite scroll).
 *    Fixed by moving hover to pure CSS (`group-hover`) — zero React
 *    re-renders on hover, and extracting `<BouquetCard>` into its own
 *    `React.memo` component so unrelated state (typing in search, view
 *    toggle) doesn't re-render cards that haven't changed.
 * 2. `animationDelay: idx * 50ms` grew without bound as more pages loaded
 *    via infinite scroll — cards on page 5 would wait over a second to
 *    appear. Capped the stagger to a small repeating window instead.
 * 3. Added `content-visibility: auto` on cards so the browser skips
 *    layout/paint for off-screen cards — this matters a lot here because
 *    infinite scroll keeps appending to the DOM with no windowing.
 * 4. Floating petals + fade-in now respect `prefers-reduced-motion`.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
const RatingStars = ({ rating }: { rating: number | string }) => {
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[...Array(5)].map((_, i) => (
        <HiStar key={i} className={`text-xs transition-colors duration-200 ${i < Math.floor(numericRating) ? "text-amber-400" : "text-gray-600"}`} />
      ))}
    </div>
  );
};
...
const DiscountBadge = ({ price }: { price: string }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1 self-start rounded-full bg-emerald-500 px-2 py-0.5 text-[10px] font-bold text-white shadow-md">
      -{discountPercent}%
    </span>
  );
};

// Fewer petals on small screens, and the whole thing disappears for anyone who prefers reduced motion.
const FloatingPetals = () => (
  <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden motion-reduce:hidden">
    {[...Array(8)].map((_, i) => (
      <div
        key={i}
        className={`absolute animate-float-petal ${i >= 4 ? "hidden sm:block" : ""}`}
        style={{
          left: `${8 + (i * 12) % 88}%`,
          top: `${-10 + (i * 5) % 20}%`,
...
// Same botanical seal used on the Shops page — keeps the two catalog pages
// reading as one brand instead of two separately-designed screens.
...
function resolveCategoryFromParam(value: string | null, categories: { id: string; slug: string; name: string }[]) {
  if (!value) return null;
  const normalized = value.trim().toLowerCase();
  if (!normalized) return null;
  return (
    categories.find((category) => {
      const candidates = [category.id, category.slug, category.name].filter(Boolean).map((candidate) => String(candidate).trim().toLowerCase());
      return candidates.includes(normalized);
    }) ?? null
  );
}

// ─── Bouquet Card (memoized, CSS-only hover — no per-hover re-renders) ─────
const BouquetCard = memo(function BouquetCard({
  ...
  // Only the first visible batch gets a staggered entrance; later infinite-scroll
  // pages appear instantly (they're already below the fold when they mount).
  const staggerDelay = idx < 9 ? (idx % 9) * 50 : 0;

  return (
    <article
      onClick={() => onNavigate(bouquetPath)}
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          onNavigate(bouquetPath);
        }
      }}
      role="button"
      tabIndex={0}
      style={{ animationDelay: `${staggerDelay}ms`, contentVisibility: "auto", containIntrinsicSize: view === "list" ? "0 300px" : "0 480px" }}
      className={`group relative cursor-pointer overflow-hidden rounded-2xl border border-[#3a1214]/60 bg-[#120708] shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-[#cb5c57]/40 hover:shadow-xl hover:shadow-[#cb5c57]/10 animate-fade-in-up motion-reduce:animate-none ${
    ...
    >
      ...
      <div className={`relative overflow-hidden bg-[#1a0809] ${view === "list" ? "lg:h-full" : ""}`}>
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onToggleFavorite(bouquet);
          }}
          className={`absolute right-2.5 top-2.5 z-10 flex h-9 w-9 items-center justify-center rounded-full border shadow-[0_8px_18px_rgba(0,0,0,0.25)] backdrop-blur-md transition-all duration-200 hover:scale-110 ${
            isFavorite
              ? "border-[#ff5b72]/60 bg-[#ff5b72]/20 text-[#ff5b72]"
              : "border-white/15 bg-[#19090a]/75 text-[#f6dacf] hover:border-[#ff7d8e] hover:bg-[#ff7d8e]/15 hover:text-[#ff8fa0]"
          }`}
          aria-label={isFavorite ? t("catalog.removeFromFavorites") : t("catalog.addToFavorites")}
        >
          {isFavorite ? <HiHeart size={15} /> : <HiOutlineHeart size={15} />}
        </button>
        ...
        <img
          ...
          className={`w-full object-cover transition-transform duration-500 ease-out group-hover:scale-105 ${
            view === "list" ? "h-[220px] lg:h-full lg:min-h-[280px]" : "h-[240px] sm:h-[280px]"
          }`}
        />
        ...
        {previewImages.length > 0 && view === "grid" && (
          <div className="absolute bottom-2.5 right-2.5 z-10 flex gap-1">
            {previewImages.map((image: string, index: number) => (
              <div key={image} className="overflow-hidden rounded-lg border border-white/15 shadow-md transition-transform duration-200 hover:scale-110">
                <img loading="lazy" src={image} alt={`${bouquet.name} ${index + 2}`} className="h-9 w-9 object-cover" />
              </div>
            ))}
          </div>
        )}

        {/* Quick view — pure CSS hover, no JS state involved */}
        <div className="absolute inset-x-0 bottom-0 z-10 p-3 pt-10 opacity-0 translate-y-4 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 group-focus-visible:opacity-100 group-focus-visible:translate-y-0">
          <button
            onClick={(e) => {
              e.stopPropagation();
              onNavigate(bouquetPath);
      ...
      <div className="flex flex-col p-4">
        <div className="flex-1 space-y-2.5">
          ...
          <Link
            ...
            className="block font-great-vibes text-[2rem] leading-tight text-[#fff3ee] transition-colors duration-200 hover:text-[#ff9b88] sm:text-[2.2rem]"
          >
            {bouquet.name}
          </Link>
          ...
          {view === "list" && bouquet.description && <p className="mt-1.5 line-clamp-2 text-sm leading-relaxed text-[#d4b8b0]">{bouquet.description}</p>}
        </div>
...
function BouquetCatalog() {
  ...
  const { register, watch, setValue } = useForm<{ search: string }>({ defaultValues: { search: searchFromParams } });
  ...
  const activeCategory = useMemo(() => resolveCategoryFromParam(categoryFromParams, categories), [categoryFromParams, categories]);
  const bouquetsQuery = useInfiniteBouquets({
    categoryId: activeCategory?.id ?? undefined,
    search: effectiveSearch || undefined,
    limit: 9,
  });
  const bouquets = useMemo(() => bouquetsQuery.data?.pages.flatMap((page) => page.items) ?? [], [bouquetsQuery.data]);
  const total = bouquetsQuery.data?.pages[0]?.total ?? 0;
  const hasActiveFilters = Boolean(activeCategory || effectiveSearch);

  const getLocalizedCategoryName = useCallback((category?: { name?: string | null; slug?: string | null } | null) => category?.name ?? "", []);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && bouquetsQuery.hasNextPage && !bouquetsQuery.isFetchingNextPage) bouquetsQuery.fetchNextPage();
      },
      { rootMargin: "400px" },
    );
  ...
  const handleToggleFavorite = useCallback(
    (bouquet: Bouquet) => {
      ...
      toast.success(result.added ? `${bouquet.name} ${t("catalog.addedToFavorites")}` : `${bouquet.name} ${t("catalog.removedFromFavorites")}`, {
  ...
  const handleAddToCart = useCallback(
    (bouquet: Bouquet) => {
      if (!isBouquetAvailable(bouquet)) {
        toast.error(`${bouquet.name} ${t("availability.outOfStockMessage")}`, { position: "bottom-right", autoClose: 1600, hideProgressBar: true });
        return;
      }
      ...
      toast.success(`${bouquet.name} ${t("catalog.addedToCart")}`, { position: "bottom-right", autoClose: 1600, hideProgressBar: true });
    },
    [t],
  );
  ...
  const renderCategoryChips = () => (
    <div className="scrollbar-none -mx-4 flex items-center gap-2 overflow-x-auto px-4 pb-1 md:mx-0 md:px-0">
      <button
        ...
        className={`shrink-0 rounded-full border px-4 py-1.5 text-[11px] font-semibold uppercase tracking-wider transition-all duration-200 active:scale-95 ${
          activeCategory === null ? "border-[#cb5c57] bg-[#cb5c57] text-white" : "border-[#5f2825]/50 bg-[#100506]/60 text-[#dfc0b8] hover:border-[#cb5c57]/60 hover:text-white"
        }`}
      >
  ...
  const renderEmptyState = () => (
    <div className="mt-10 rounded-2xl border border-dashed border-[#5f2825]/40 bg-[#0f0506]/60 px-6 py-16 text-center">
      <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full border border-[#5f2825]/30 bg-[#1a0c0c]">
      ...
      <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-[#c9aba4]">{t("catalog.noBouquetsDesc")}</p>
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Browse premium flower bouquets, seasonal arrangements, and fresh floral gifts online." />
      </Helmet>

      <main className="relative isolate min-h-screen overflow-hidden text-[#fff6f4]">
        {/* Background */}
        <div className="pointer-events-none fixed inset-0 -z-20">
          <img src={bouquetsPageBackground} alt="" aria-hidden="true" fetchPriority="high" className="h-full w-full object-cover object-center opacity-80" />
        </div>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mb-8 text-center">
              <h1 className="font-great-vibes text-[clamp(3.2rem,7vw,6rem)] leading-[0.9] text-[#fff6f1] [text-shadow:0_8px_28px_rgba(0,0,0,0.5)]">
                ...
                <span className="block text-[#ff9b88]">{activeCategory ? t("catalog.bouquetsCollection") : t("catalog.perfectBouquet")}</span>
              </h1>

              <PressedFlowerDivider className="mx-auto mt-4 max-w-xs" />

              <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#f0d8d0]/80">
                {activeCategory ? t("catalog.categoryHeroDescription", { category: getLocalizedCategoryName(activeCategory) }) : t("catalog.defaultHeroDescription")}
              </p>
            ...
            {/* ── Sticky Filters Bar ── */}
            <div
              className={`sticky top-14 z-20 mb-6 rounded-2xl border border-[#3a1214]/60 bg-[#0d0405]/90 p-3 backdrop-blur-xl transition-shadow duration-300 sm:top-16 ${
            ...
            >
              <div className="space-y-3">
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  {/* View toggle */}
                  <div className="inline-flex self-start rounded-xl border border-[#5f2825]/50 bg-[#090304]/80 p-1 sm:self-auto">
                    <button
                      type="button"
                      onClick={() => setView("grid")}
                      className={`flex h-9 w-9 items-center justify-center rounded-lg transition-all duration-150 ${
                        view === "grid" ? "bg-[#cb5c57] text-white" : "text-[#ad8d85] hover:bg-[#2b1012] hover:text-white"
                      }`}
                    ...
                    <button
                      ...
                      className={`flex h-9 w-9 items-center justify-center rounded-lg transition-all duration-150 ${
                        view === "list" ? "bg-[#cb5c57] text-white" : "text-[#ad8d85] hover:bg-[#2b1012] hover:text-white"
                      }`}
                  ...
                  {/* Result count pill */}
                  <div className="inline-flex items-center gap-2 self-start rounded-xl border border-[#5f2825]/40 bg-[#090304]/80 px-4 py-2.5 sm:self-auto">
                    <FaCheckCircle size={10} className="shrink-0 text-emerald-400" />
                    <span className="text-xs text-[#c9a09a]">{t("catalog.results")}</span>
                    <span className="text-sm font-bold text-white">
                      <span className="text-[#ff9b88]">{bouquets.length}</span>
                      <span className="mx-1 text-[#5a3a36]">/</span>
                      {total}
                    </span>
                  </div>
                </div>
            ...
            {hasActiveFilters && (
              <div className="mb-4 flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[#b88d84]">
                  <FaFilter size={8} className="mr-1 inline" />
                ...
                <button type="button" onClick={clearAllFilters} className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] text-[#c9a09a] transition-colors hover:text-white">
                  <HiXMark size={11} />
            ...
            {bouquetsQuery.isLoading ? (
              <BouquetGridSkeleton count={6} className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3" imageClassName="h-[240px] w-full rounded-t-2xl" />
            ) : bouquets.length ? (
              <div className={`grid gap-5 ${view === "grid" ? "grid-cols-1 sm:grid-cols-2 xl:grid-cols-3" : "grid-cols-1"}`}>
                {bouquets.map((bouquet, idx) => (
                  <BouquetCard
                    ...
                    idx={idx}
                    view={view}
                    isFavorite={favoriteIds.has(bouquet.id)}
                    categoryName={getLocalizedCategoryName(bouquet.category)}
              ...
              <div className="mt-6 flex justify-center">
                <div className="inline-flex items-center gap-3 rounded-full border border-[#5f2825]/40 bg-[#0f0506]/80 px-5 py-2.5 backdrop-blur-sm">
                  <div className="flex gap-1">
                    {[0, 150, 300].map((delay) => (
                      <span key={delay} className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#ff9b88]" style={{ animationDelay: `${delay}ms` }} />
                    ))}
                  </div>
        ...
        <style>{`
          ...
          @media (prefers-reduced-motion: reduce) {
            .animate-fade-in-up, .animate-float-petal {
